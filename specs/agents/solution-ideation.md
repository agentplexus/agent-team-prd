...
tasks:
  - id: generate-options
    description: Generate 2-4 viable solution options
    type: manual
    required: true
    expected_output: Multiple solution options proposed

  - id: map-to-problems
    description: Map each option to problems it addresses
    type: manual
    required: true
    expected_output: Problem-solution mapping complete

  - id: analyze-tradeoffs
    description: Identify tradeoffs and risks per option
    type: manual
    required: true
    expected_output: Tradeoffs documented

  - id: define-non-goals
    description: Explicitly define non-goals
    type: manual
    required: true
    expected_output: Non-goals documented

  - id: recommend-option
    description: Recommend preferred option with rationale
    type: manual
    required: true
