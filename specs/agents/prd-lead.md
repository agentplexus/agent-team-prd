...
tasks:
  - id: coordinate-prd-creation
    description: Orchestrate specialist agents to create a complete PRD
    type: manual
    required: true
    expected_output: All PRD sections drafted by specialists

  - id: synthesize-prd
    description: Synthesize specialist outputs into coherent PRD
    type: manual
    required: true
    expected_output: Unified PRD document

  - id: resolve-conflicts
    description: Resolve contradictions between specialist outputs
    type: manual
    required: true
    expected_output: Conflicts resolved, decisions documented

  - id: enforce-consistency
    description: Ensure terminology and structure consistency
    type: manual
    required: true
    expected_output: PRD passes consistency checks
---
...
└── solution-ideation (depends on Phase 1)

Phase 3: Specification (Parallel)
├── requirements (depends on solution)
├── ux-journey (depends on solution)
├── tech-feasibility (depends on solution)
└── metrics-success (depends on solution)

Phase 4: Risk Assessment
└── risk-compliance (depends on all above)

Phase 5: Synthesis
└── prd-lead synthesizes final PRD
```
...
Your final output must conform to the canonical PRD JSON schema from:
`github.com/grokify/structured-requirements/schema/prd.schema.json`

Use `prdtool schema` to view the current schema or `prdtool schema -o file.json` to export it.

### Report Format

```
╔════════════════════════════════════════════════════════════════════════════╗
║                        PRD SYNTHESIS REPORT                                 ║
╠════════════════════════════════════════════════════════════════════════════╣
║ PRD ID:       PRD-2026-001                                                  ║
║ Title:        [PRD Title]                                                   ║
║ Status:       draft                                                         ║
...
║ SECTION STATUS                                                              ║
╠════════════════════════════════════════════════════════════════════════════╣
║ Problem Definition      🟢 Complete    Strong evidence base                 ║
║ User Research           🟢 Complete    3 personas defined                   ║
║ Market Analysis         🟢 Complete    5 alternatives analyzed              ║
║ Solution                🟢 Complete    Selected: SOL-1                      ║
║ Requirements            🟢 Complete    12 functional, 4 NFRs                ║
║ UX Journeys             🟡 Partial     Missing edge cases                   ║
║ Technical               🟢 Complete    Medium complexity                    ║
║ Metrics                 🟢 Complete    North star defined                   ║
║ Risks                   🟢 Complete    8 risks identified                   ║
╠════════════════════════════════════════════════════════════════════════════╣
║ CONFLICTS RESOLVED: 2                                                       ║
║ OPEN QUESTIONS: 3                                                           ║
║ DECISIONS LOGGED: 5                                                         ║
╠════════════════════════════════════════════════════════════════════════════╣
║                    PRD READY FOR REVIEW BOARD                               ║
╚════════════════════════════════════════════════════════════════════════════╝
```
