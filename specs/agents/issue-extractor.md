...
tasks:
  - id: extract-issues
    description: Extract issues from Review Board outputs
    type: manual
    required: true
    expected_output: Normalized issue list

  - id: deduplicate
    description: Deduplicate overlapping issues
    type: manual
    required: true
    expected_output: Issues deduplicated

  - id: assign-severity
    description: Assign severity to each issue
    type: manual
    required: true
    expected_output: Severity assigned

  - id: map-to-sections
    description: Map issues to PRD sections
    type: manual
    required: true
    expected_output: Section mapping complete

  - id: assign-owners
    description: Assign issues to appropriate agents
    type: manual
    required: true
    expected_output: Owners assigned
---
